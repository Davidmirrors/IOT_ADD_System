...
u8 MTF01_Date;					 		//MTF01接收数据解析
//u8 Mtf_Date_Flag = 0;					 //接收标志位
//初始化IO 串口2
//bound:波特率
void uart2_init(u32 bound)														//PA2 USART2_TX  PA3 USART_RX
{
   //GPIO端口设置
  GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
	...
	//USART2端口配置
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2 | GPIO_Pin_3; //GPIOA PA2与GPIOA PA3
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//速度50MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
...
{  
	...
	va_end(ap);
	i=strlen((const char*)USART2_RX_BUF);		//此次发送数据的长度
	for(j=0;j<i;j++)							//循环发送数据
	{
		while(USART_GetFlagStatus(USART2,USART_FLAG_TC)==RESET); //循环发送,直到发送完毕   
		USART_SendData(USART2,USART2_RX_BUF[j]); 
	} 
}
